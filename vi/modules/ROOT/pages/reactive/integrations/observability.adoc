...
Bạn cũng có thể chỉ vô hiệu hóa các observation của Security cho một tập con.
...
2. `spring.security.http.chains.before` - một span bọc phần tiếp nhận của các filter bảo mật
3. `spring.security.http.chains.after` - một span bọc phần trả về của các filter bảo mật
